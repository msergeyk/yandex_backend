...
import json
from controllers.couriers_controller import (
    create_couriers,
    update_courier,
    validate_courier_id,
    is_courier_id_exists,
)
...
def load_couriers_list():
    ...
    if status == 400:
        not_val_ids = [
            request_data[x]["courier_id"] for x in not_validated_idx
        ]
        id_msg = [
            {"id": c_id, "message": error_messages[i]}
            for i, c_id in enumerate(not_val_ids)
        ]
        validated_data = [
            d for i, d in enumerate(request_data) if i not in not_validated_idx
        ]
...
def complete():
    ...
    status, result = complete_order(
        courier_id=courier_id, order_id=order_id, complete_time=complete_time
    )
    if status == 400:
        return Response(status=400)
    else:
