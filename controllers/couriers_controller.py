...
def update_courier(data, courier_id):
    ...
    if c:
        ...
        session.query(Courier
            ).filter(Courier.courier_id == courier_id
            ).update({Courier.courier_type: courier_type,
                    Courier.regions: regions,
            Courier.working_hours: cast(working_hours, ARRAY(TSRANGE))},
            synchronize_session=False)
        session.commit()
        active_orders = session.query(Order
        ...
        if len(active_orders) > 0:
            ...
            for act_ord in active_orders:
                if not ((act_ord.weight <= courier.weight)
            ...
            if len(orders_to_update) > 0:
                session.query(Order
                ).filter(Order.order_id.in_(orders_to_update)
